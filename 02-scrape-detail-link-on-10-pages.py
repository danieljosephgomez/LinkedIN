...
def login_and_search():
    ...
    email.send_keys('finddanielgomez@gmail.com')

    time.sleep(3)

    password = driver.find_element_by_xpath('//*@id="session_password-login"')
    password.send_leys('s-p*_GY.ECn\G8k:')
    ...
    search.send_key('python programmer')

    time.sleep(3)

    button = driver.find_element_by_xpath('//*="global-search]/fieldset/button')
    button.click()
...
def get_page_links():
    ...
    for a in ul.find_all('a')[0:(len(ul.find_all('a'))-2)]:
        print('https://www.linklinkedin.com' + a['href'])
